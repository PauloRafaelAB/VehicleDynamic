...
def chassis(parameters: Initialization, logger: logging.Logger, wx: float, wy: float, wz: float, ax: float, ay: float, az: float):
    ...
    logger.info(f" position_chassi_force {np.shape(parameters.position_chassi_force[0,:])} strut2chassi_xyz {parameters.strut2chassi_xyz[:,0]}")
    ...
    logger.debug(f"shape sum_crossproduct_r_f {np.shape(sum_crossproduct_r_f)} sum_crossproduct {sum_crossproduct_r_f}")
...
def main():
    ...
    for i in range_calc:
        ...
        parameters.x_rf.wheel_forces_transformed_force2vehicle_sys = np.array([[30,30,30,30], #[sim_data[i].wheel_load_x_FL, sim_data[i].wheel_load_x_RL, sim_data[i].wheel_load_x_FR, sim_data[i].wheel_load_x_RR],
                                                                               [0,0,0,0], #[sim_data[i].wheel_load_y_FL, sim_data[i].wheel_load_y_RL, sim_data[i].wheel_load_y_FR, sim_data[i].wheel_load_y_RR],
